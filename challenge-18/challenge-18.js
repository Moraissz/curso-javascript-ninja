(function(){
 ...
 /*    cpf.forEach(function(value){
    ...
    */
    ...
    var lastQuestiontext = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
    var regexLastQuestion = /(<(\w+)>)([\wáéí\s]+)(<\/\w+>)/g;
    var html = function html(capturaTotal,opentag,nametag,text,closetag){
        console.log(opentag + 'O texto dentro da tag "' + nametag + '" é "' + text + '"'+closetag);
        console.log('\n');
    }
    
    console.log(lastQuestiontext.replace(regexLastQuestion,html));
})();